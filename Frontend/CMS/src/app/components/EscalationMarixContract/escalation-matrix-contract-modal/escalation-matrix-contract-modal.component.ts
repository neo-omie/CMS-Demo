...
import { GetMasterEscalationMatrixContractByIdDto, UpdateMatrixContractDto } from '../../../models/escalation-matrix-contract';
...
export class EscalationMatrixContractModalComponent {
  ...
  constructor(private escalationService: EscalationMatrixContractService, private renderer: Renderer2, private approverMatrixContractService: ApproverMatrixContractService) { }
    ...
    if (nod1 !== "" && Number(nod1) > 0 &&
      ...
      this.escalationService.postMatrixContractById(id, updateMatrixContractDto).subscribe({
        next: (response: any) => {
          ...
          const modalElement = document.getElementById('escalation-matrix-contract-modal');
          if (modalElement) {
